...
def read_recipe_user_data(recipe_id):
	...
	try:
		...
		if 'user' in session:
			favorite_recipes = session['user'].get()['favorite_recipes']
			liked = session['user'].get()['liked_recipes']

			if recipe_id in favorite_recipes:
				is_favorite = True
			if recipe_id in liked:
				is_liked = True
		return jsonify(favorite=is_favorite, liked=is_liked, rating=recipe_rating), 200
	except Exception as e:
...
@recipes_bp.route('/recipe/remove_from_favorites/<string:recipe_id>', methods=['GET'])
@login_required
...
@recipes_bp.route('/recipe/remove_from_liked/<string:recipe_id>', methods=['GET'])
@login_required
