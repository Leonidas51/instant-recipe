...
def save_recipe_photo(file, recipe_id, recipe_published):
    ...
    user = session['user']
    upload = {}
    upload['uploader_id'] = user.get_entry()['_id']
    upload['uploader_name'] = user.get_entry()['name']
    ...
    user_update_data = {}
    user_update_data['upload_images'] = user.get_entry()['upload_images']
    user_update_data['upload_images'].append(image_id)
    user.update_in_db(user_update_data)
    user.set_from_db_by_email(user.get_entry()['email'])
...
def suggest_recipe():
    if request.method == 'POST':
        try:
            ...
            recipe['tag_ids'] = []
            recipe['tag_names'] = []
            recipe['author'] = session['user'].get_entry()['name']
            ...
            user = session['user']
            user_entry = user.get_entry()
            user_update_data = {
                'upload_recipes': user_entry['upload_recipes']
            }
            user_update_data['upload_recipes'].append(recipe_id)
            user.update_in_db(user_update_data)
            user.set_from_db_by_email(user_entry['email'])
...
def add_to_favorites(recipe_id):
    ...
    try:
        data = {}
        user = session['user']
        data['favorite_recipes'] = user.get_entry()['favorite_recipes']
...
def add_to_liked(recipe_id):
    ...
    try:
        ...
        user = session['user']
        user_id = user.get_id()
        data['liked_recipes'] = user.get_entry()['liked_recipes']
...
def delete_own_recipe():
    ...
    try:
        recipe_id = ObjectId(request.json.get_entry('recipe_id'))
        user = session['user']
        if recipe_id in user.get_entry()['upload_recipes']:
            user_uploads = user.get_entry()['upload_recipes']
